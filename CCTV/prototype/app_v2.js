/**
 * IBVAP v2 Command & Control Dashboard — app_v2.js
 *
 * Based on app.js with the following v2 enhancements:
 *  - VIRTUAL_FENCE_INTRUSION & SUSPICIOUS_LOITERING treated as person/human threats
 *  - SUSPICIOUS_VEHICLE treated as vehicle threat
 *  - Purple colour scheme for SUSPICIOUS_VEHICLE
 *  - Blockchain "ON-CHAIN SECURED" badge with deterministic visual hash
 *  - Clean label formatting (underscores → spaces)
 *
 * Responsibilities:
 *  - Live clock & date
...
function updateStats(events) {
  const persons  = events.filter(e => isPerson(e)).length;
  const vehicles = events.filter(e => isVehicle(e)).length;
  ...
  if (events.length > 0) {
    ...
    const lastLabel = last.event_type.replace(/_/g, ' ');
    $statLast.innerHTML =
      `<span class="text-text">${lastLabel}</span><br/>` +
...
// ── Card Builder ──────────────────────────────────────────────────────────────
// v2: VIRTUAL_FENCE_INTRUSION and SUSPICIOUS_LOITERING are human threats
function isPerson(ev) {
  return ev.event_type === 'PERSON_DETECTED'
      || ev.event_type === 'VIRTUAL_FENCE_INTRUSION'
      || ev.event_type === 'SUSPICIOUS_LOITERING';
}

// v2: SUSPICIOUS_VEHICLE is a vehicle threat
function isVehicle(ev) {
  return ev.event_type === 'VEHICLE_DETECTED'
      || ev.event_type === 'SUSPICIOUS_VEHICLE';
}

function getEventColour(ev) {
  // v2: Granular colour mapping for new event types
  if (ev.event_type === 'VIRTUAL_FENCE_INTRUSION') {
    return { border: 'border-red',   badge: 'bg-red/15 text-red border-red/40',     bar: 'bg-red',          flash: 'alert-flash' };
  }
  if (ev.event_type === 'SUSPICIOUS_LOITERING') {
    return { border: 'border-amber', badge: 'bg-amber/15 text-amber border-amber/40', bar: 'bg-amber',       flash: 'alert-flash-amber' };
  }
  if (ev.event_type === 'SUSPICIOUS_VEHICLE') {
    return { border: 'border-[#d946ef]', badge: 'bg-[#d946ef]/15 text-[#d946ef] border-[#d946ef]/40', bar: 'bg-[#d946ef]', flash: 'alert-flash-purple' };
  }
  // Original types
...
function buildCard(ev, isNew = false) {
  ...
  // v2: Clean label — replace all underscores with spaces
  const label = ev.event_type.replace(/_/g, ' ');

  // v2: Deterministic visual hash from event UUID
  const visualHash = '0x' + ev.id.replace(/-/g, '').substring(0, 14);
  ...
  card.innerHTML = `
    ...
    <div class="p-3 space-y-2.5">
      ...
      <!-- v2: Blockchain ON-CHAIN SECURED badge -->
      <div class="flex items-center justify-between pt-2 mt-1 border-t border-border">
        <span class="font-mono text-neon text-[10px] tracking-wider flex items-center gap-1">
          <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zM11 17l-5-5 1.41-1.41L11 14.17l7.59-7.59L20 8l-9 9z"/></svg> ON-CHAIN SECURED
        </span>
        <span class="font-mono text-subtext text-[10px]">${visualHash}</span>
      </div>
...
function applyFilter() {
  ...
  cards.forEach(card => {
    ...
    const show = activeFilter === 'ALL'
      || (activeFilter === 'PERSON'  && isPerson(ev))
      || (activeFilter === 'VEHICLE' && isVehicle(ev));
...
console.log(
  '%cIBVAP v2 C&C Dashboard%c loaded — polling every %dms | blockchain UI active',
